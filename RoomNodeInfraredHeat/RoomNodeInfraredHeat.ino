...
uint8_t RoomTargetTemp = 74;
const int RiseAboveTarget = 2;
const int FallBelowTarget = 2;
...
void receive(const MyMessage& message) {
    ...
    if (message.type == V_HUM && message.sensor == TortHumdId) {
        saveState(TortHumdPos, message.bValue);
    }
    if (message.type == V_TEMP && message.sensor == RoomTempId) {
        saveState(RoomTempPos, message.bValue);
    }
}
